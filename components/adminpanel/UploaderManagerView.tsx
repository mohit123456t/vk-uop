...
const UploaderManagerView = () => {
    const [uploaders, setUploaders] = useState<UserProfile[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedUploader, setSelectedUploader] = useState<UserProfile | null>(null);
    ...
    const handleViewProfile = (uploader: UserProfile) => {
                    ...
                    {uploaders.map(uploader => (
                        <div key={uploader.uid} className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className={`px-2 py-1 rounded-full text-xs font-medium ${uploader.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                    {uploader.isActive ? 'Active' : 'Inactive'}
                                </div>
                            </div>

                            <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                                <div>
                                    <div className="text-2xl font-bold text-slate-800">{uploader.assignedTasks || 0}</div>
                                    <div className="text-xs text-slate-500">Assigned</div>
                                </div>
                                <div>
                                    <div className="text-2xl font-bold text-yellow-600">{uploader.pendingTasks || 0}</div>
                                    <div className="text-xs text-slate-500">Pending</div>
                                </div>
                                <div>
                                    <div className="text-2xl font-bold text-green-600">{uploader.completedTasks || 0}</div>
                                    <div className="text-xs text-slate-500">Completed</div>
                                </div>
                            </div>

                            <div className="text-sm text-slate-600 mb-4">
                                <strong>Last Activity:</strong> {uploader.lastLoginAt ? new Date(uploader.lastLoginAt).toLocaleString() : 'Never'}
                            </div>

                            <div className="flex space-x-2">
                                <button onClick={() => handleViewProfile(uploader)} className="flex-1 bg-blue-600 text-white px-3 py-2 rounded-md text-sm font-medium hover:bg-blue-700 transition-colors">
                                    View Details
                                </button>
                                <button onClick={() => handleOpenAssignTaskModal(uploader)} className="flex-1 bg-green-500 text-white px-3 py-2 rounded-md text-sm font-medium hover:bg-green-600 transition-colors">
                                    Assign Task
                                </button>
                            </div>
                        </div>
                 ...
                 <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
                    <div className="bg-slate-50 rounded-2xl shadow-2xl p-8 max-w-2xl w-full transform transition-all">
                        <div className="flex items-start justify-between mb-6">
                            <div className="flex items-center">
                                <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mr-4 border-2 border-white shadow-md">
                                    <span className="text-emerald-600 font-bold text-2xl">
                                        {selectedUploader.name?.charAt(0)?.toUpperCase() || 'U'}
                                    </span>
                                </div>
                                <div>
                                    <h2 className="text-3xl font-bold text-slate-800">{selectedUploader.name}'s Profile</h2>
                                    <p className="text-slate-600">{selectedUploader.role}</p>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200/80">
                                ...
                                <p className="text-3xl font-bold text-slate-800 mt-1">{selectedUploader.assignedTasks || 0}</p>
                            </div>
                            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200/80">
                                <p className="text-sm font-medium text-slate-500">Pending Tasks</p>
                                <p className="text-3xl font-bold text-yellow-600 mt-1">{selectedUploader.pendingTasks || 0}</p>
                            </div>
                            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200/80">
                                <p className="text-sm font-medium text-slate-500">Completed Tasks</p>
                                <p className="text-3xl font-bold text-green-600 mt-1">{selectedUploader.completedTasks || 0}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200/80">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="text-slate-800 font-mono">{selectedUploader.email}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="font-medium text-slate-600">Status:</span>
                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${selectedUploader.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                        {selectedUploader.isActive ? 'Active' : 'Inactive'}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="font-medium text-slate-600">Last Login:</span>
                                    <span className="text-slate-800">{selectedUploader.lastLoginAt ? new Date(selectedUploader.lastLoginAt).toLocaleString() : 'Never'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
