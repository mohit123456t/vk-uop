...
const ThumbnailMakerManagerView = () => {
    const [thumbnailMakers, setThumbnailMakers] = useState<ThumbnailMaker[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedMaker, setSelectedMaker] = useState<ThumbnailMaker | null>(null);
        ...
        const fetchThumbnailMakers = async () => {
            ...
            querySnapshot.forEach((doc) => {
                if (doc.data().role === 'thumbnail-maker') {
                    makers.push({ ...doc.data(), id: doc.id } as ThumbnailMaker);
                }
            });
    ...
    const handleViewProfile = (maker: ThumbnailMaker) => {
    ...
    return (
        <div className="p-6 bg-slate-50 min-h-screen">
            <div className="mb-8">
                ...
                <p className="text-slate-600">Manage and monitor thumbnail maker performance</p>
            </div>
                ...
                <table className="min-w-full leading-normal">
                    ...
                    <tbody>
                        {thumbnailMakers.map((maker) => (
                            <tr key={maker.id} className="hover:bg-gray-50">
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p className="text-gray-900 whitespace-no-wrap">{maker.name}</p>
                                ...
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <span className={`relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight`}>
                                        <span aria-hidden className={`absolute inset-0 ${maker.status === 'Active' ? 'bg-green-200' : 'bg-red-200'} opacity-50 rounded-full`}></span>
                                        <span className="relative">{maker.status}</span>
                                    </span>
                                </td>
                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                    <button onClick={() => handleViewProfile(maker)} className="text-indigo-600 hover:text-indigo-900 font-semibold">View Profile</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {selectedMaker && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
                    <div className="bg-white rounded-lg shadow-2xl p-8 max-w-2xl w-full">
                        <h2 className="text-2xl font-bold text-slate-800 mb-4">Thumbnail Maker Profile</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-700">Personal Details</h3>
                                <p className="text-slate-600 mt-2"><strong>Name:</strong> {selectedMaker.name}</p>
                                <p className="text-slate-600"><strong>Email:</strong> {selectedMaker.email}</p>
                            </div>
                            <div>
