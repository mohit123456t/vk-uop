...
const VideoEditorManagerView = () => {
    ...
    const role = 'video_editor';
    const field = 'assignedEditor';
    const taskCollectionName = 'campaigns';
    ...
    const fetchStaffData = useCallback(async () => {
        ...
        try {
            ...
            const enrichedStaff = await Promise.all(staffList.map(async (staff) => {
                const tasksQuery = query(collection(db, taskCollectionName), where(field, '==', staff.uid));
                const tasksSnapshot = await getDocs(tasksQuery);
                const stats: StaffStats = {
                    assigned: tasksSnapshot.docs.length,
                    pending: tasksSnapshot.docs.filter(doc => ['Active', 'In-Progress', 'Pending Footage'].includes(doc.data().status)).length,
                    completed: tasksSnapshot.docs.filter(doc => doc.data().status === 'Approved' || doc.data().status === 'Submitted').length,
                };
                return { ...staff, stats };
            }));
            setVideoEditors(enrichedStaff);
        } catch (error) {
            console.error(`Failed to fetch video editors or their stats:`, error);
        } finally {
            setLoading(false);
        }
    }, []);
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-6 text-center bg-white/30 p-3 rounded-lg">
                             ...
                             <div className="flex space-x-3">
                                <button onClick={() => {
                                  ...
                                  const tasksQuery = query(collection(db, taskCollectionName), where(field, '==', editor.uid));
                                  getDocs(tasksQuery).then(snapshot => {
                                    setStaffCampaigns(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
                                  });
                    ...
                    <motion.div className="fixed inset-0 bg-black/75 backdrop-blur-md flex items-center justify-center z-50 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <motion.div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md border border-slate-300/70" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}>
                            ...
                            <form className="p-6" onSubmit={async (e) => {
                                ...
                                await updateDoc(doc(db, 'campaigns', campaignId), { assignedEditor: selectedStaff.uid });
